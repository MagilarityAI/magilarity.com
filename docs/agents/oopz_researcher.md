...
**Role:** Processes PPOU (Procurement Appeals Body) decisions and builds the regulatory precedent database

---

## Purpose

Ukraine's Procurement Appeals Body (PPOU / ООПЗ) issues decisions on procurement complaints. These decisions form the primary regulatory precedent for what is and isn't lawful in tenders.
...
| 6 | Legal validation — verify all statutory references against legislation DB |
...
## 🛡️ Anti-Hallucination System — Two Factors
...
All statutory references are verified against the full Ukrainian legislation database (~286,634 normative documents in `appdb`). References to non-existent articles are flagged.
...
| 5.1 | Complainant's position | Arguments + legal basis |
| 5.2 | Buyer's counterarguments | Buyer's objections |
| 5.3 | PPOU decision | Reasoning, key questions, logic, norms applied |
...
| 9 | **Analysis metadata** | Technical information |

---

## 🗄️ Database Output

Decisions are stored in `agent_memory` with full structured data and also indexed in `appdb.oopz_decisions` for retrieval by other agents:
...
```
4,400+  PPOU decisions available for processing
~30 sec  processing time per decision
...
| Agent | How it uses PPOU data |
|-------|----------------------|
| `tender_doc_researcher` | Retrieves relevant precedents by CPV code to inform analysis |
| `bid_researcher` | References AMCU practice on composite document acceptance |
...
The PPOU decisions database is a **unique asset** that cannot be quickly replicated:

```
4,400+ structured PPOU decisions
        ↓
Patterns of typical buyer violations
        ↓
Success rate statistics for different complaint types
        ↓
Precedent base for predicting
appeal outcomes
        ↓
More decisions = more accurate conclusions
from tender_doc_researcher
```

This is a **self-improving system** — every newly analyzed PPOU decision improves the quality of analysis for all subsequent tenders.
